...
void pqsort(void* a){
    ...
    if(args->depth >= args->max_depth){
        qsort(args->arr, (args->right - args->left), sizeof (int), (int(*) (const void *, const void *)) comp);
    }else{
        submit_qsort_task(args->depth+1, args->max_depth, args->left, new_border, args->arr, args->pool);
        submit_qsort_task(args->depth+1, args->max_depth,new_border, args->right, args->arr, args->pool);
    }
}
